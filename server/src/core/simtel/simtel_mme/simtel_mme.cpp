...
void SimtelMme::trySendReport(unsigned int smsId,
  ...
  while (!reportSent) {
    ...
    if (warningSec) {
      std::string warningSecStr = std::to_string(*warningSec);
      MessageHolder::instance().addInfoMsg(
          "SMS(" + uid.toStr() + ") TTL: " + warningSecStr + " seconds left");
    }
...
std::expected<common::imsi_t, std::string> SimtelMme::handleAttachRequestMTimsi(
    const common::imsi_t &mTimsi, const common::imsi_t &realImsi,
    const common::imei_t &imei, unsigned int mmeId) {
  MessageHolder::instance().addMsg(
...
SimtelMme::handleAttachRequestImsi(const common::imsi_t &imsi,
                                   const common::imei_t &imei) {
  MessageHolder::instance().addMsg(createLogMsg("received imsi is real imsi"));
...
SimtelMme::handleAttachRequest(const common::imsi_t &imsi,
  ...
  if (!isRealImsi) {
    return handleAttachRequestMTimsi(imsi, realImsi, imei, mmeId);
  } else {
    return handleAttachRequestImsi(imsi, imei);
  }
}
...
std::optional<std::string> SimtelMme::handleUssd(const common::imsi_t &mTimsi,
  ...
  case common::UssdCode::GetBalance: {
    ...
    return bs->sendUssdBalance(mTimsi, *balance);
  }
  case common::UssdCode::GetPhoneNumber: {
    ...
    return bs->sendUssdMsisdn(mTimsi, *msisdn);
  }
  default:
    return "Unknown command";
  }
...
void SimtelMme::trySendSms(const common::msisdn_t &msisdn_s, unsigned int smsId,
  ...
  while (true) {
    ...
    bs->sendSmDelivery(mtimsi_d, smsId, msisdn_s, binText);
