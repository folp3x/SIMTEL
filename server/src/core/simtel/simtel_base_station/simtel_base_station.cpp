...
#include "core/simtel/simtel_ue_context/simtel_ue_context.h"
...
std::optional<std::string> SimtelBaseStation::handleLocationUpdate(
  ...
  common::Location<> ueLoc{locReq.getCoords()};
  auto error = sendSignalLevels(locReq.getImei(), ueLoc, ctx);
  if (error) {
    return *error;
  }
  ...
  } else if (chosenBsReq->getImei() != locReq.getImei()) {
  ...
  } else if (configureConfirm->getMTimsi() != ctx->getMTimsi()) {
...
std::optional<std::string>
SimtelBaseStation::sendSignalLevels(const common::imei_t &imei,
                                    const common::Location<> &ueLoc,
                                    std::shared_ptr<SimtelUeContext> ctx) {
  for (const auto &[id, bs] : baseStations) {
    unsigned int signalLevel = bs->measureSignal(ueLoc);
...
std::optional<std::string>
SimtelBaseStation::handleRequestFromBuf(std::shared_ptr<SimtelUeContext> ctx) {
  ...
  default:
    return "Unexpected request: " + common::requestTypeToStr(*reqType);
  }
...
std::optional<std::string> SimtelBaseStation::handleMeasurementReport(
  ...
  if (!canAcceptConnection()) {
    ...
    auto response = std::make_unique<common::ErrorResponse>(error);
    auto sendError = sendResponse(ctx, std::move(response));
    return sendError ? "Error sending error info: " + *sendError : error;
  }
  ...
  if (!mTimsi) {
    ...
    auto sendError = sendResponse(ctx, std::move(response));
    return sendError ? "Error sending error info: " + *sendError
                     : mTimsi.error();
  }
...
void SimtelBaseStation::handleUeRequests(std::shared_ptr<SimtelUeContext> ctx) {
  ...
  while (true) {
    ...
    if (receiveError) {
      ...
      MessageHolder::instance().addMsg("");
      auto error = handleRequestFromBuf(ctx);
      if (error) {
