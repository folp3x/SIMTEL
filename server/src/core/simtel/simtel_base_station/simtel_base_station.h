...
#include "core/simtel/simtel_mme/simtel_mme.h"
#include "core/simtel/simtel_ue_context/simtel_ue_context.h"
#include "core/ttl_manager/ttl_manager.h"
...
private:
  ...
  static std::optional<std::string>
  sendSignalLevels(const common::imei_t &imei, const common::Location<> &ueLoc,
                   std::shared_ptr<SimtelUeContext> ctx);

  static std::shared_ptr<SimtelBaseStation> findBs(unsigned int id);

  std::optional<std::string>
  handleRequestFromBuf(std::shared_ptr<SimtelUeContext> ctx);
