...
std::unique_ptr<BaseJsonInfo> MeasurementControlResponse::getJsonRootInfo() {
  auto root = makeJsonObject();

  root->addInner("imei",
                 makeJsonValue<imsi_t>(&imei, Validator::isCorrectImsi));

  root->addInner("signal", makeJsonValue<unsigned int>(
                               &signal, Validator::isCorrectSignal));
