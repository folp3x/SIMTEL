...
template <typename T> std::string_view MenuItemMove<T>::getName() const {
  return "move";
}

template <typename T> int MenuItemMove<T>::getArgsCount() {
...
template <typename T> std::vector<T> MenuItemMove<T>::getCoords() const {
  return coords;
}
} // namespace client
